...
import { CircleMarker, GeoJSON, MapContainer, Polyline, Popup } from "react-leaflet";
...
function ImpactReportDownload({ siteId }: { siteId: string }) {
  const [status, setStatus] = useState<"idle" | "loading" | "error">("idle");
  async function download() { try { setStatus("loading"); const response = await fetch(`http://localhost:8000/api/v1/intervention-sites/${siteId}/impact-report`); if (!response.ok) throw new Error("Report unavailable"); const url = URL.createObjectURL(await response.blob()); const link = document.createElement("a"); link.href = url; link.download = `jaldirshti-impact-report-${siteId}.pdf`; link.click(); URL.revokeObjectURL(url); setStatus("idle"); } catch { setStatus("error"); } }
  return <span className="report-control"><button className="report-download" onClick={download} disabled={status === "loading"}>{status === "loading" ? "Preparing report…" : "Download Impact Report"}</button>{status === "error" ? <small>Local report service is unavailable. Start the FastAPI service and retry.</small> : null}</span>;
}
...
export function WatershedMap({ evidence, demoScenarioVersion }: { evidence: EvidenceResult | null; demoScenarioVersion: number }) {
  ...
  const [recommendationState, setRecommendationState] = useState<"idle" | "loading" | "error" | "ready">("idle");
  ...
  useEffect(() => {
    ...
    setRecommendationState("loading"); setRecommendation(null);
    fetch(`http://localhost:8000/api/v1/risk-zones/${selectedRisk.id}/recommendations`).then((response) => response.ok ? response.json() : Promise.reject()).then((data: RecommendationResult) => { setRecommendation(data); setRecommendationState("ready"); }).catch(() => { setRecommendation(null); setRecommendationState("error"); });
  }, [selectedRisk]);
  useEffect(() => { if (!demoScenarioVersion || !layers) return; setSelected(layers.interventions.features[0].properties); setSelectedRisk(layers.riskZones.features[0].properties); }, [demoScenarioVersion, layers]);
  const selectedSummary = useMemo(() => selected ? <>{selected.name ?? selected.intervention_type} selected · <ImpactReportDownload siteId={selected.id} /></> : "Select a marker for field details", [selected]);
  ...
  return <section className="map-shell" aria-label="Watershed GIS map">
    ...
    <div className="map-grid"><div className="map-canvas"><span className="offline-map-label">OFFLINE BASEMAP · LOCAL PILOT LAYERS</span><MapContainer center={demoCenter} zoom={14} scrollWheelZoom className="leaflet-map" aria-label="Map of the illustrative demo watershed">
    ...
    </MapContainer>{loadError ? <div className="map-state map-state--error">Could not load the local map layers. Check the frontend data folder and reload.</div> : !layers ? <div className="map-state">Loading prepared local map layers…</div> : null}</div>
